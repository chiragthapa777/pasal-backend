...
module.exports = {
	...
	async addProduct(req) {
		try {
			...
			const data = await prisma.cartDetails.create({
				data: {
					productId: Number(productId),
	...
	async removeProduct(req) {
		try {
			...
			const data = await prisma.cartDetails.update({
				where: {
					id: find[0].id,
				},
				data: {
	...
	async editProduct(req) {
		try {
			...
			if (Number(quantity) !== find.quantity) {
				await verifyProductReq(productId, quantity);
				data = await prisma.cartDetails.update({
					where: {
						id: find[0].id,
					},
					data: {
				...
				throw "No change in the item was found"
			}
			return data;
		} catch (error) {
			throw error;
